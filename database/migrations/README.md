...
| `019_anon_privacy_and_rate_limit_identity.sql` | **Security fix — NOT YET APPLIED.** (a) Column-level SELECT on `ratings` so `metadata` (the anonymous reviewer's device fingerprint) is no longer world-readable — verified exposed via `GET /rest/v1/ratings?select=metadata`; adds `get_anon_rating_id()` so the client can still find its own row. (b) Rebuilds `enforce_rate_limit()`: identity now uses the **rightmost** x-forwarded-for hop and is never NULL, closing a fail-open where a blank first hop skipped every cap silently; extends the ratings trigger to UPDATE. **Deploy the app change first** (`useRatings.ts` stops selecting `metadata`) |
