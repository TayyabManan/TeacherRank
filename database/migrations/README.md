...
| `016_drop_dead_objects.sql` | **NOT APPLIED — soak gate EXPIRED ~2026-07-17, still pending as of 2026-08-05.** Drops superseded RPC overloads, the unused `ratings_with_info` view, `query_performance_logs`, and zero-scan indexes. Re-run the audit in the file header before applying: the `teachers` table went 127 → 362 rows on 2026-08-05 (Bahria import), so any "zero-scan index" conclusion drawn against the smaller table is stale |
...
| `019_anon_privacy_and_rate_limit_identity.sql` | **Security fix — STILL NOT APPLIED, re-verified against prod 2026-08-05.** (a) Column-level SELECT on `ratings` so `metadata` (the anonymous reviewer's device fingerprint) is no longer world-readable; adds `get_anon_rating_id()` so the client can still find its own row. (b) Rebuilds `enforce_rate_limit()`: identity now uses the **rightmost** x-forwarded-for hop and is never NULL, closing a fail-open where a blank first hop skipped every cap silently; extends the ratings trigger to UPDATE. **The app-side prerequisite already shipped (commit 39193fd), so this is unblocked — nothing is waiting on it but the run.** Live evidence it is still open: `GET /rest/v1/ratings?select=metadata` → 200 with fingerprints on 15/15 anon rows (14 devices), and `POST /rest/v1/rpc/get_anon_rating_id` → 404 PGRST202. Side effect of the delay: that 404 makes the anon re-review dedupe silently no-op, so a cooldown expiry can insert a duplicate rating (0 have occurred so far) |
