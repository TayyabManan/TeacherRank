...
| `022_bayesian_rating_sort.sql` | **UX fix — applied 2026-08-27 via the Supabase dashboard** (independently of the 021/020/016 runs; it needs only 012+013). "Top Rated" (`rating_desc`) ordered by raw `avg_rating`, so a 5.0 from 3 reviews outranked a 4.8 from 50 (verified live 2026-08-27: the home page's #1 was a 3-review 5.0; Baymard: ~70% of users trust 4.5×180 over 4.8×39 — volume is credibility). Replaces the `rating_desc`/`rating_asc` ORDER BY inside `get_teachers_with_stats` with an IMDb-style Bayesian score `(v·R + m·C)/(v+m)` (m=5, C = review-weighted platform mean computed inline from `teachers`; unrated teachers stay pinned to the bottom of Top Rated). Displayed averages, the RPC signature, and `get_teachers_count` are unchanged — no client deploy needed; the listing's #N rank badges are position-derived and follow the new order automatically (`rating_asc` mirrors so the badge countdown stays correct). Verification queries at the bottom double as the m-tuning loop |
...
Confirmed applied: **008–015, 017, 018, 019**; **022** applied 2026-08-27.
